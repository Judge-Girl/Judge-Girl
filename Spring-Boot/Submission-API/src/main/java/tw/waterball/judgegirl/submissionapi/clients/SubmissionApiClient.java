...
import tw.waterball.judgegirl.commons.exceptions.SubmissionNotFoundException;
...
@ServiceDriver
@Component
public class SubmissionApiClient extends BaseRetrofitServiceAPI implements SubmissionServiceDriver {
    private API api;

    @Autowired
    public SubmissionApiClient(RetrofitFactory retrofitFactory,
                               @Value("${judge-girl.submission-service.address}") String address,
                               @Value("${judge-girl.submission-service.port}") int port) {
    ...
    @Override
    public Submission getSubmission(String token, int problemId, int studentId, String submissionId) throws SubmissionNotFoundException {
        return errorHandlingGetBody(() -> api.getSubmission("Bearer " + token,
                problemId, studentId, submissionId).execute());
    }

    @Override
    public FileResource getZippedSubmittedCodes(String token, int problemId, int studentId, String submissionId) throws SubmissionNotFoundException {
