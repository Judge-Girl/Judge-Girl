...
public class GroupsHomeworkProgressPresenter implements GetGroupsHomeworkProgressUseCase.Presenter {
    ...
    private List<GetGroupsHomeworkProgressUseCase.StudentSubmissionRecord> records;
    ...
    public StudentsHomeworkProgress present() {
        StudentsHomeworkProgress groupsHomeworkProgress = new StudentsHomeworkProgress();
        var studentRecords = toMap(records, GetGroupsHomeworkProgressUseCase.StudentSubmissionRecord::getStudentId, this::getStudentProgress);
        students.forEach(student -> groupsHomeworkProgress.addProgress(student.getEmail(), studentRecords.get(student.getId())));
        return groupsHomeworkProgress;
    }

    private StudentsHomeworkProgress.StudentProgress getStudentProgress(GetGroupsHomeworkProgressUseCase.StudentSubmissionRecord record) {
