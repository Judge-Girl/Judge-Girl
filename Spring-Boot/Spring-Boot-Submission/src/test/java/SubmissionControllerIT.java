...
public class SubmissionControllerIT {
    ...
    void WhenSubmitCodeWithValidToken_ShouldSaveIt_DeployJudger_ListenToVerdictIssuedEvent_AndHandleTheEvent() throws Exception {
        ...
        amqpTemplate.convertAndSend(SUBMISSION_EXCHANGE_NAME,
                String.format("%s.%s.judge", SUBMISSION_EXCHANGE_NAME, submissionView.getId()),
                objectMapper.writeValueAsString(verdictIssuedEvent));

        // Verify the submission is updated with the judge response
        submitCodeUseCase.waitForNextJudgeCompletedEventLock.doWait(3000);
