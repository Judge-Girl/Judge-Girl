...
void log_write(int level, const char *source_filename, const int line, const char *submission_id, const FILE *log_fp, const char *fmt, ...) {
    ...
    strftime(datetime, 99, "%Y-%m-%d %H:%M:%S", timeinfo);
    ...
    int count = snprintf(buffer, log_buffer_size,
                         "[submission_%s] %s [%s] [%s:%s]%s\n",
                          submission_id, LOG_LEVEL_NOTE[level], datetime, source_filename, line_str, log_buffer);
