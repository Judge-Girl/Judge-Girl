...
int _cgcreate(char *controller, char *group, uid_t uid, gid_t gid) {
    char cmd[MAX_PATH_LENGTH] = "", cgroup_path[MAX_PATH_LENGTH];
    struct stat st = {0};
    snprintf(cgroup_path, sizeof(cgroup_path), "/sys/fs/cgroup/%s/%s", controller, group);

    if (stat(cgroup_path, &st) == 0) {
        snprintf(cmd, sizeof(cmd), "cgdelete %s:%s", controller, group);
        system(cmd);
    }    
    snprintf(cmd, sizeof(cmd), "cgcreate -g %s:%s", controller, group);
    if (system(cmd) == 0) {
        snprintf(cmd, sizeof(cmd), "chown -R %u:%u /sys/fs/cgroup/%s/%s", uid, gid, controller, group);
        if (system(cmd) == 0) {
            return 0;
        }
    } else {
...
int _cgset_memory(char *controller, char *group, long long max_memory) {
    ...
    snprintf(cmd, sizeof(cmd), "cgset -r memory.limit_in_bytes=%lld %s", max_memory, group);
    if (system(cmd) == 0) {
        return 0;
    } else {
        return -1;
...
void child_process(FILE *log_fp, struct config *_config, int mount_list_len) {
    ...
    */
    ...
    /*
    char *controller = "memory", *group = "sandbox";
    if (_cgcreate(controller, group, _config->uid, _config->gid) != 0) {
        CHILD_ERROR_EXIT(CGROUP_FAILED);
    }
    if (_config->memory_limit_check_only == 0) {
        if (_config->max_memory != UNLIMITED) {
            if(_cgset_memory(controller, group, _config->max_memory) != 0) {
                CHILD_ERROR_EXIT(CGROUP_FAILED);
            }
        }
    }
    */
    ...
    // set max process number limit; set max thread number limit
    ...
    if (strcmp(_config->error_path, "")) {
        // if outfile and error_file is the same path, we use the same file pointer
        if (strcmp(_config->output_path, "") && strcmp(_config->output_path, _config->error_path) == 0) {
    ...
    execvp(_config->args[0], _config->args);
